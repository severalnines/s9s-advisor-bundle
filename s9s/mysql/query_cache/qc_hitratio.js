...
function main()
{
    ...
    for (idx = 0; idx < hosts.size(); ++idx)
    {
        ...
        var Qcache_hits = host.sqlStatusVariable("Qcache_hits");
        var Qcache_inserts = host.sqlStatusVariable("Qcache_inserts");
        var query_cache_type = host.sqlSystemVariable("query_cache_type");
        if (Qcache_hits.isError() ||
            Qcache_inserts.isError() ||
            query_cache_type.isError())
        {
        ...
        else
        {
            if (checkPrecond(host))
            {
                    ...
                    query_cache_type == "OFF")
                {
                    ...
                    {
                        msg = "Not enough data to calculate";
                        justification = "The Query cache is disabled.";
                    }
                    else
                    {
                        msg = "Not enough data to calculate";
