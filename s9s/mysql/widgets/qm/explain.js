            ...
            if (ret != false && ret.size() > 0)
            {
                ...
                for (i=0; i<ret.size(); ++i)
                {
                    ...
                    else
                    {
                        ...
                        result[k]["select_type"]=ret[i][1];
                        result[k]["table"]=ret[i][2];
                        result[k]["partitions"]=ret[i][3];
                        result[k]["type"]=ret[i][4];
                        result[k]["possible_keys"]=ret[i][5];
                        result[k]["key"]=ret[i][6];
                        result[k]["key_len"]=ret[i][7];
                        result[k]["ref"]=ret[i][8];
                        result[k]["rows"]=ret[i][9];
                        result[k]["filtered"]=ret[i][10];
                        result[k]["extra"]=ret[i][11];
                    }
                    k++;
                }
            }
