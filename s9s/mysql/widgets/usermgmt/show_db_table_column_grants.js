...
function main(mydb, table, user, hostname, hostAndPort)
{
    ...
    result["accounts"] = {};
    var query = "";
    query = "SELECT grantee as account, table_schema as db, table_name as `table`, column_name as `column`, "
    + "group_concat(privilege_type separator ',') as privileges, group_concat(is_grantable separator ',') as grantable "
    ...
    query += " group by grantee, table_schema, table_name, column_name order by grantee asc";
    ...
    for (idx = 0; idx < hosts.size(); ++idx)
    {
        ...
        if (retval != false)
        {
            ...
            for (row = 0; row < nRows; ++row)
            {
                ...
                column               = resultSet[row, 3];
                privs                = resultSet[row, 4];
                grantable            = resultSet[row, 5];
                ...
                result["accounts"][idx][i]["db.table"]             = db + "." + table;
                result["accounts"][idx][i]["table.column"]         = column;
