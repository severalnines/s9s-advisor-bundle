...
function main(hostAndPort) {
    ...
    for (idx = 0; idx < hosts.size(); ++idx) {
        ...
        result["innodb_status"][index] = {};
        result["innodb_status"][index]["host"]={};
        result["innodb_status"][index]["host"]["hostname"] = host.hostName();
        result["innodb_status"][index]["host"]["port"] = host.port();
        ...
        if (connected) {
            ...
            if (ret != false && ret.size() > 0)
            {
                ...
                {
                    result["innodb_status"][index][i] =  ret[i][2];
            ...
            {
               result["innodb_status"][index][i] = "No data";
            }
            print("</table>");
